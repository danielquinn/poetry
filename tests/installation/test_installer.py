...
def test_run_with_dependencies(installer, locker, repo, package, f):
    package_a = get_package("A", "1.0")
    package_b = get_package("B", "1.1")
    repo.add_package(package_a)
    repo.add_package(package_b)

    package.add_dependency(f.create_dependency("A", "~1.0"))
    package.add_dependency(f.create_dependency("B", "^1.0"))
...
def test_run_update_after_removing_dependencies(
    installer, locker, repo, package, installed, f
):
    locker.locked(True)
    ...
    package.add_dependency(f.create_dependency("A", "~1.0"))
    package.add_dependency(f.create_dependency("B", "~1.1"))
...
def test_run_install_no_dev(installer, locker, repo, package, installed, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
    ...
    package.add_dependency(f.create_dependency("A", "~1.0"))
    package.add_dependency(f.create_dependency("B", "~1.1"))
    package.add_dependency(f.create_dependency("C", "~1.2", category="dev"))
...
def test_run_install_remove_untracked(installer, locker, repo, package, installed, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
...
def test_run_whitelist_add(installer, locker, repo, package, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
...
def test_run_whitelist_remove(installer, locker, repo, package, installed, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
...
def test_add_with_sub_dependencies(installer, locker, repo, package, f):
    package_a = get_package("A", "1.0")
    package_b = get_package("B", "1.1")
    package_c = get_package("C", "1.2")
    ...
    package_a.add_dependency(f.create_dependency("D", "^1.0"))
    package_b.add_dependency(f.create_dependency("C", "~1.2"))
...
def test_run_with_python_versions(installer, locker, repo, package, f):
    ...
    package.add_dependency(f.create_dependency("A", "~1.0"))
    package.add_dependency(f.create_dependency("B", "^1.0"))
    package.add_dependency(f.create_dependency("C", "^1.0"))
...
def test_run_with_optional_and_python_restricted_dependencies(
    installer, locker, repo, package, f
):
    package.python_versions = "~2.7 || ^3.4"
    ...
    package_c13.add_dependency(f.create_dependency("D", "^1.2"))
    ...
    package.add_dependency(
        f.create_dependency("A", {"version": "~1.0", "optional": True})
    )
    package.add_dependency(
        f.create_dependency("B", {"version": "^1.0", "python": "~2.4"})
    )
    package.add_dependency(
        f.create_dependency("C", {"version": "^1.0", "python": "~2.7 || ^3.4"})
    )
...
def test_run_with_optional_and_platform_restricted_dependencies(
    installer, locker, repo, package, mocker, f
):
    ...
    package.add_dependency(
        f.create_dependency("B", {"version": "^1.0", "platform": "custom"})
    )
    package.add_dependency(
        f.create_dependency("C", {"version": "^1.0", "platform": "darwin"})
    )
...
def test_run_with_dependencies_extras(installer, locker, repo, package, f):
    package_a = get_package("A", "1.0")
    ...
    package_b.add_dependency(
        f.create_dependency("C", {"version": "^1.0", "optional": True})
    )
    ...
    package.add_dependency(f.create_dependency("A", "^1.0"))
    package.add_dependency(
        f.create_dependency("B", {"version": "^1.0", "extras": ["foo"]})
    )
...
def test_run_does_not_install_extras_if_not_requested(
    ...
    package.add_dependency(f.create_dependency("C", "^1.0"))
    package.add_dependency(
        f.create_dependency("D", {"version": "^1.0", "optional": True})
    )
...
def test_run_installs_extras_if_requested(installer, locker, repo, package, f):
    package.extras["foo"] = [get_dependency("D")]
    package_a = get_package("A", "1.0")
    package_b = get_package("B", "1.0")
    package_c = get_package("C", "1.0")
...
def test_run_installs_extras_with_deps_if_requested(
    ...
    package_c.add_dependency(f.create_dependency("D", "^1.0"))
...
def test_installer_with_pypi_repository(package, locker, installed, config, f):
    pool = Pool()
    ...
    package.add_dependency(f.create_dependency("pytest", "^3.5", category="dev"))
    installer.run()
...
def test_run_installs_with_local_file(installer, locker, repo, package, f):
    file_path = fixtures_dir / "distributions/demo-0.1.0-py2.py3-none-any.whl"
    package.add_dependency(f.create_dependency("demo", {"file": str(file_path)}))
...
def test_run_installs_wheel_with_no_requires_dist(installer, locker, repo, package, f):
    file_path = (
...
def test_run_installs_with_local_poetry_directory_and_extras(
    installer, locker, repo, package, tmpdir, f
):
...
def test_run_installs_with_local_setuptools_directory(
    ...
    package.add_dependency(f.create_dependency("my-package", {"path": str(file_path)}))
...
def test_run_with_prereleases(installer, locker, repo, package, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
    ...
    package.add_dependency(
        f.create_dependency("A", {"version": "*", "allow-prereleases": True})
    )
    package.add_dependency(f.create_dependency("B", "^1.1"))
...
def test_run_changes_category_if_needed(installer, locker, repo, package, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
    ...
    package_a = get_package("A", "1.0")
    package_b = get_package("B", "1.1")
    package_b.add_dependency(f.create_dependency("A", "^1.0"))
    repo.add_package(package_a)
    repo.add_package(package_b)

    package.add_dependency(
        f.create_dependency("A", {"version": "^1.0", "optional": True}, category="dev")
    )
    package.add_dependency(f.create_dependency("B", "^1.1"))
...
def test_run_update_all_with_lock(installer, locker, repo, package, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
    ...
    package.add_dependency(f.create_dependency("A", "*"))
...
def test_run_update_with_locked_extras(installer, locker, repo, package, f):
    locker.locked(True)
    locker.mock_lock_data(
        {
            "package": [
    ...
    package.add_dependency(
        f.create_dependency("A", {"version": "^1.0", "extras": ["foo"]})
    )
    package.add_dependency(f.create_dependency("D", "^1.0"))
...
def test_run_install_duplicate_dependencies_different_constraints(
    ...
    package_a.add_dependency(
        f.create_dependency("B", {"version": "^1.0", "python": "<4.0"})
    )
    package_a.add_dependency(
        f.create_dependency("B", {"version": "^2.0", "python": ">=4.0"})
    )

    package_b10 = get_package("B", "1.0")
    package_b20 = get_package("B", "2.0")
    package_b10.add_dependency(f.create_dependency("C", "1.2"))
    package_b20.add_dependency(f.create_dependency("C", "1.5"))
...
def test_run_update_uninstalls_after_removal_transient_dependency(
    ...
    package_a = get_package("A", "1.0")
    package_a.add_dependency(
        f.create_dependency("B", {"version": "^1.0", "python": "<2.0"})
    )
...
def test_run_install_duplicate_dependencies_different_constraints_with_lock_update(
    ...
    package_a = get_package("A", "1.1")
    package_a.add_dependency(f.create_dependency("B", "^2.0"))
...
def test_installer_test_solver_finds_compatible_package_for_dependency_python_not_fully_compatible_with_package_python(
    ...
    package.python_versions = "~2.7 || ^3.4"
    package.add_dependency(
        f.create_dependency("A", {"version": "^1.0", "python": "^3.5"})
    )
...
def test_installer_required_extras_should_not_be_removed_when_updating_single_dependency(
    installer, locker, repo, package, installed, env, pool, config, f
):
    package.add_dependency(f.create_dependency("A", {"version": "^1.0"}))
...
def test_installer_required_extras_should_not_be_removed_when_updating_single_dependency_pypi_repository(
    locker, repo, package, installed, env, mocker, config, f
):
    mocker.patch("sys.platform", "darwin")
    ...
    package.add_dependency(f.create_dependency("poetry", {"version": "^0.12.0"}))
    ...
    package.add_dependency(f.create_dependency("pytest", "^3.5"))
...
def test_installer_required_extras_should_be_installed(
    locker, repo, package, installed, env, config, f
):
    pool = Pool()
    pool.add_repository(MockRepository())
...
def test_update_multiple_times_with_split_dependencies_is_idempotent(
    ...
    a11.add_dependency(f.create_dependency("B", ">=1.0.1"))
    a11.add_dependency(f.create_dependency("C", {"version": "^1.0", "python": "~2.7"}))
    a11.add_dependency(f.create_dependency("C", {"version": "^2.0", "python": "^3.4"}))
...
def test_installer_can_install_dependencies_from_forced_source(
    locker, package, installed, env, config, f
):
    package.python_versions = "^3.7"
    package.add_dependency(
        f.create_dependency("tomlkit", {"version": "^0.5", "source": "legacy"})
    )
...
def test_run_installs_with_url_file(installer, locker, repo, package, f):
    url = "https://python-poetry.org/distributions/demo-0.1.0-py2.py3-none-any.whl"
    package.add_dependency(f.create_dependency("demo", {"url": url}))
...
def test_installer_uses_prereleases_if_they_are_compatible(
    installer, locker, package, repo, f
):
    package.python_versions = "~2.7 || ^3.4"
    package.add_dependency(
        f.create_dependency(
    ...
    package_b.add_dependency(f.create_dependency("prerelease", ">=0.19"))
    ...
    package.add_dependency(f.create_dependency("b", "^2.0.0"))
...
def test_installer_can_handle_old_lock_files(
    installer, locker, package, repo, installed, config, f
):
    pool = Pool()
    pool.add_repository(MockRepository())
